    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {memberDatabase.map((member) => (
            <div
              key={member.id}
              className="bg-[#1f1f1f] rounded-lg p-6 transform hover:scale-105 transition-all duration-300 hover:shadow-[0_0_30px_rgba(176,219,156,0.6)]"
            >
              <div className="relative mb-4">
                <img
                  src={member.photo}
                  alt={member.name}
                  className="w-24 h-24 rounded-full mx-auto object-cover border-2 border-[#b0db9c]"
                />
              </div>

              <div className="text-center">
                <h3 className="text-xl font-semibold mb-2">{member.name}</h3>
                <p className="text-[#b0db9c] mb-4">{member.position}</p>
